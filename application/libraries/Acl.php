...
class Acl {
	 ...
	 * @return boolean
	 */
	public function has_permission($controller, $required_permissions = array('delete all'), $authors_uid = NULL)
	{
		...
		foreach ($permissions as $action => $role)
		{
			if (in_array($action, $required_permissions))
			{
				...
				if($authors_uid !== NULL){
					foreach ($authors_uid as $author_uid)
					{

						if (($action == 'edit own' OR $action == 'delete own') && ( isset($author_uid) && $author_uid == $uid))
						{
							return TRUE;
						}
