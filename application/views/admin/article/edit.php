...
<h4><?php echo $article->status == 2 ? 'Published' : 'Draft ' . $article->status; ?></h4>
<table class="table">
  
  <input type="hidden" name="created" value="<?php echo $article->created ?>" />
  <tr>
  ...
  <tr>
    <td>Prehead Title</td>
    <td><?php echo form_input('pre_head', set_value('pre_head', $article->pre_head)); ?></td>
  </tr>

	<tr>
		<td>Posthead Title</td>
		<td><?php $data = array('name'=>'title', 'value' => $article->title, 'id' => 'title', 'onkeyup' => 'sync()', 'class' => 'input-lg'); echo form_input($data); ?></td>
	</tr>
	<tr>
		<td>Slug</td>
		<td><?php echo form_input('slug', set_value('slug', $article->slug),'id="slug"'); ?></td>
	</tr>
	<tr>
		<td>Author</td>
	...
	<tr>
		<td>Body</td>
		<td><?php echo form_textarea('body', html_entity_decode(set_value('body', $article->body)), 'class="ckeditor" id="textarea-ckeditor"'); ?></td>
	</tr>
  <tr>
		<td>Summary</td>
		<td><?php echo form_textarea('summary', set_value('summary', $article->summary), 'class="textarea"'); ?></td>
	</tr>
	
	<!-- <tr>
		<td>Posthead</td>
		<td><?php echo form_input('post_head', set_value('post_head', $article->post_head)); ?></td>
	</tr> -->
  <tr>
		<td>Tagline</td>
		<td><?php echo form_dropdown('tagline', $tagline, set_value('pre_head', $article->tagline), 'id="tagline"')?>
	...
	<tr>
		...
		<td><?php echo form_input('ireport_indicator', set_value('ireport_indicator', $article->ireport_indicator)); ?></td>
	</tr>
  <?php if ($is_data_staff) { ?>
  <tr>
    <td>Index</td>
    <td><?php echo form_input('index_num', set_value('index_num', $article->index_num),'id="index"'); ?></td>
  </tr>
  <?php } ?>
	<tr>
		<td>Tags</td>
		<td><?php echo form_dropdown('tags[]', $tags, set_value('tags[]', $selected_tags), 'id="tags" onChange="addMultiple(this, \'tags[]\', 3);" class="inputdd"')?>
		<button type="button" id="moreTagsButton" class="btn btn-default form-btn" onclick="showCreateTag()">
	      Add More Tags
	    </button>
		<?php echo form_input('new_tag', '', 'id="tagInputBar" style="display:none;" class="addtext"'); ?>
		<button type="button" id="tagInputButton" class="btn btn-default form-btn" style="display:none" onclick="addText(0, 'newtags[]', 3)">
	      Create
	    </button>
		<ul>
  ...
  <?php } ?>
	...
	<tr>
		<td>General Notes</td>
		<td><?php echo form_textarea('general_notes', set_value('general_notes', $article->general_notes), 'class="textarea"'); ?></td>
	</tr>

  <tr>
		<td>Series</td>
		<td><?php echo form_dropdown('series[]', $series, set_value('series[]', $selected_series), 'id="series" onChange="addMultiple(this, \'series[]\', 5);" class="inputdd"')?>
		<button type="button" id="moreSeriesButton" class="btn btn-default form-btn" onclick="showCreateSeries()">
	      Create New Series
	    </button>
		<?php echo form_input('new_series', '', 'id="seriesInputBar" style="display:none;" class="addtext"'); ?>
		<?php if ($is_data_staff) { ?>
    <button type="button" id="seriesInputButton" class="btn btn-default form-btn" style="display:none" onclick="addText(2, 'newseries[]', 5)">
	      Create
	    </button>
    <?php } else { ?>
     <button type="button" id="seriesInputButton" class="btn btn-default form-btn" style="display:none" onclick="addText(1, 'newseries[]', 4)">
        Create
      </button>
    <?php } ?>
		<ul>
		<?php foreach($selected_series as $series) { ?>
		 <li onclick="this.parentNode.removeChild(this);">
		  <input type="hidden" name="series[]" value="<?php echo $series ?>" />
		   <?php echo get_series_name($series); ?> x
		 </li>
		 <?php } ?>
		</ul>
		</td>
  ...
  <tr>
    ...
    <td><?php echo form_input('series_indicator', set_value('series_indicator', $article->series_indicator)); ?></td>
  </tr>
  ...
  <tr class="Upload">
    ...
    <td><?php echo form_dropdown('files[]', $files, set_value('files[]', ''), 'id="files" class="inputdd"')?>
      ...
      <input type = "file" multiple id="upload" name = "new_files[]" size = "20" class="form-btn"/>          
    </td>
  ...
  <?php }}?>
	...
	<tr>
		<td></td>
    <td><button name="status" value="<?php echo $article->status?>" class="btn btn-primary">Save</button></td>

    <?php if($is_editor && $article->status == 1) { ?>
...
{
  // code from http://odyniec.net/articles/multiple-select-fields/
 ...
 var ul = document.getElementsByTagName('ul')[ul_rank];
  ...
  for (var i = 0; i < choices.options.length; i++) {
    if (choices.options[i].innerHTML == text.value) {
    	...
    	var div = document.createElement('div');
		div.className = "alert alert-warning"
		var textNode = document.createTextNode("Already exists.");
		div.appendChild(textNode);
		ul.parentNode.insertBefore(div, ul);
      return;
    }
...
{
  var seriesButton = document.getElementById('moreSeriesButton');
  var inputBar = document.getElementById('seriesInputBar');
  var inputButton = document.getElementById('seriesInputButton');
